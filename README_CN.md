...
从 [Releases](https://github.com/nanmi/cc-portkey/releases) 下载对应平台的二进制文件，放到 PATH 目录中。
...
```bash
cc-portkey current
# 输出: deepseek (DeepSeek)
```

### `cc-portkey show [profile]`

显示 profile 详细配置（API Key 脱敏）。
...
```bash
cc-portkey link              # 默认创建到 ~/.local/bin/
cc-portkey link /usr/local/bin  # 指定目录
```
